...
**データベース不要・日付計算のみ**の要件に合わせ、不要な依存関係を削除した軽量構成です。

### 削除した機能
- ❌ SQLite3（データベース不要）
- ❌ Active Storage（画像アップロード不要）
- ❌ Solid系（Cache/Queue/Cable - DB依存機能）
- ❌ Action Mailer関連（メール送信不要）
...
- `kamal`, `thruster` - デプロイツール（MVP不要）
- `bundler-audit`, `brakeman`, `rubocop-rails-omakase` - セキュリティ・静的解析（開発初期は優先度低）
...
### ❌ 削除したもの
- **データベース関連**: Active Record, SQLite3
- **ストレージ関連**: Active Storage, storage.yml
- **キャッシュ/ジョブ**: Solid Cache/Queue/Cable
- **メール関連**: Action Mailer（既にAPI modeで削除済み）
- **デプロイツール**: Kamal, Thruster
- **セキュリティ監査**: Bundler Audit, Brakeman, Rubocop
